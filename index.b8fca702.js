...
/* eslint-disable brace-style */ /* eslint-disable prettier/prettier */ /* eslint-disable no-unused-expressions */ "use strict";
...
function initializeGame() {
    ...
    document.addEventListener("touchend", (e)=>{
        ...
        const isEnoughDistance = Math.max(Math.abs(deltaX), Math.abs(deltaY)) > minSwipeDistance;
        if (!isEnoughDistance) return;
        if (isHorizontal) deltaX > 0 ? game.moveRight() : game.moveLeft();
        else deltaY > 0 ? game.moveDown() : game.moveUp();
        updateUI();
        game.getStatus();
    });
}
